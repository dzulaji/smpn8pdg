...
@section('content')
    <div class="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p class="text-gray-600">Daftar sarana dan prasarana penunjang kegiatan sekolah.</p>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Nama Fasilitas</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($fasilitas as $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <div class="w-16 h-10 bg-gray-200 rounded-sm flex items-center justify-center text-gray-400 text-xs">No Img</div>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-yellow-100 text-yellow-800 px-2.5 py-1 rounded-sm text-xs font-bold border border-yellow-200">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('admin.fasilitas.destroy', $item->id) }}" method="POST" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-gray-500 text-sm">Belum ada data fasilitas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        function confirmDelete(id) {
            Swal.fire({
                ...
                text: "Fasilitas, foto utama, dan seluruh foto galeri akan dihapus permanen dari server!",
                icon: 'error',
