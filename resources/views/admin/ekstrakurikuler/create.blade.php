...
@section('header_title', 'Tambah Ekstrakurikuler Baru')
...
@section('content')
    ...
    <div class="bg-white border border-gray-200 rounded-sm shadow-sm p-6 max-w-4xl">
        <form id="form-ekskul" action="{{ route('admin.ekstrakurikuler.store') }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="deskripsi_singkat" value="{{ old('deskripsi_singkat') }}"
                    class="w-full bg-gray-50 border border-gray-300 rounded-sm px-4 py-2.5 text-gray-800 focus:outline-none focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400 text-sm"
                    placeholder="Ditampilkan di halaman index (Max 255 karakter)" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Galeri Tambahan (Opsional)</label>
