...
@section('page_content')
    ...
    <div class="mb-6">
        ...
        <div class="flex items-center gap-3 text-sm text-ink-muted border-y border-border py-3">
            ...
            <span>Ditulis oleh <strong>{{ $prestasi->penulis ?? 'Administrator' }}</strong></span>
        </div>
    </div>
    ...
    <div class="border-t border-border pt-6 mt-10 flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <button onclick="navigator.clipboard.writeText(window.location.href); alert('Link berhasil disalin!');"
                class="w-10 h-10 rounded-full bg-surface-1 border border-border flex items-center justify-center text-ink hover:text-white hover:bg-ink transition-colors cursor-pointer"
                title="Salin Link">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}" target="_blank"
                class="w-10 h-10 rounded-full bg-surface-1 border border-border flex items-center justify-center text-ink hover:text-white hover:bg-[#1877F2] transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M22.675 0h-21.35c-.732 0-1.325.593-1.325 1.325v21.351c0 .731.593 1.324 1.325 1.324h11.495v-9.294h-3.128v-3.622h3.128v-2.671c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12v9.293h6.116c.73 0 1.323-.593 1.323-1.325v-21.35c0-.732-.593-1.325-1.325-1.325z" />
                </svg>
            </a>
            <a href="https://api.whatsapp.com/send?text={{ urlencode('Prestasi: ' . $prestasi->judul . ' ' . request()->url()) }}"
                target="_blank"
                class="w-10 h-10 rounded-full bg-surface-1 border border-border flex items-center justify-center text-ink hover:text-white hover:bg-[#25D366] transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                    <path
