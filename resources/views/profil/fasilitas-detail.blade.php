...
@section('title', $fasilitas->judul)

@section('parent_breadcrumb')
    <a href="/profil/fasilitas" class="hover:text-primary transition-colors">Fasilitas Sekolah</a>
@endsection
...
@section('page_content')
    ...
    <div class="aspect-[21/9] w-full bg-surface-2 rounded-xl border border-border mb-8 overflow-hidden shadow-sm">
        @if ($fasilitas->foto_utama)
            <img src="{{ asset('storage/' . $fasilitas->foto_utama) }}" alt="{{ $fasilitas->judul }}"
    ...
    <div class="prose prose-ink max-w-none mb-12 leading-relaxed text-justify">
        <h2 class="text-2xl font-bold text-ink mb-4">Tentang {{ $fasilitas->judul }}</h2>
        {!! $fasilitas->deskripsi_lengkap !!}
    </div>

    <!-- Galeri Carousel Multi-Item + Lightbox -->
    @if (is_array($fasilitas->galeri) && count($fasilitas->galeri) > 0)
        <h3 class="text-xl font-bold text-ink mb-4 border-b border-border pb-2">Galeri {{ $fasilitas->judul }}</h3>
    ...
    <script>
        ...
        function openLightbox(imageSrc) {
            ...
            // Sedikit delay biar efek transisi opacity & scale jalan
            setTimeout(() => {
        ...
        if (container) {
            function scrollNext() {
                ...
                if (container.scrollLeft >= maxScroll - 10) {
                    ...
                    const itemWidth = container.querySelector('div').clientWidth + 16; // 16px itu gap
                    container.scrollBy({
                        left: itemWidth,
                        behavior: 'smooth'
                    });
            ...
            // Berhenti otomatis kalau kursor diarahkan ke area galeri
